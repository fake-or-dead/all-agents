...
|---|---|---|
| Applicant report + eight-sheet XLSX | Eight sheets: alumni male, new male, alumni female, new female, monk, nun, staff male, staff female. Include submitted through completed states, grouped by approved persona/category; numeric seniority ascending, Thai name, then immutable application ID. | Reporting user with `report.read` and per-field grants. Screen/XLSX may include separately granted sensitive classes. Print always omits mental-health and substance-use. |
| Teacher report + print | Five groups are derived from linked application persona + gender + approved category; supplied group is never authoritative and a conflict is rejected/audited. Invited through completed only. Thai name then immutable ID; maximum 10 sheets. | Assigned teacher with `print.create`; emergency-contact, health, and medication require their exact read/print grants. National ID, mental-health, and substance-use always omitted. |
| Laundry / facilities | Confirmed through completed only; approved-category rank, room, Thai name, then immutable laundry ID. Days `01`–`08`, costs, totals, and persisted needs. | Operations or reporting user with scope and field grants. Screen, print, XLSX use identical rows and totals. |

The legacy report/export disagreement is intentionally resolved by the above rules; it is not copied. Thai ordering executes NFC normalization, outer-space trimming, internal-space collapse, `Intl.Collator('th')` primary comparison, then immutable ID. Conflict fixtures prove applicant seniority outranks name, laundry category outranks room/name, room outranks name, and ID resolves equal normalized names. Dates accept `YYYY-MM-DD` and render `dd/MM/BBBB` with `+543`. Independently authored golden row values exist for applicant screen/print/XLSX, counters, teacher print, and laundry screen/print/XLSX; the validator compares computed outputs to them. Sensitive fixtures exercise authorized/denied behavior for every class and surface. Output request, artifact generation, print rendering, download, expiration, and failure are audited.
...
The server-issued challenge and companion assertion bind `iss`, `kid`, `jti`, `nonce`, `actor_account_id`, `session_id`, `action=identity.verify`, `course_session_id`, `aud=tapoda-thai-id-companion`, and exact `origin`. The deterministic verifier fixtures cover valid, unknown issuer/key, invalid signature, expiry, audience/action/origin/actor/session/course mismatch, `jti`/nonce mismatch, and replay. `jti` and nonce are consumed together only after every check passes. Each computed decision is compared to an independent expected decision and PII-free audit outcome; rejected cases cannot partially consume either value.
